...
$Id: replay.py 43 2013-12-31 00:35:22Z true $
'''
...
# set up additional things before we start
if (len(sys.argv) >= 4):
    if (ftype == '.fm2'):
...
ser.read()			# wait for next byte before sending version command
ser.write("~v")		# poll for version
time.sleep(0.005) 	# and wait a moment (USB fix)
