...
#define STATUS 5   // analog pin 0
#define SD_LOAD 9  // analog pin 4
        ...
        case HIGH:
            ...
            while (true)
            {
                ...
                if (frame > 1 && frame % 512 == 1 && data_read == false) { 
                    sd_card.read_data_eeprom(frame+1023);
                    data_read = true;
                }
                else if (frame % 512 != 1)
                {
            ...
            while (Serial.available()>0)
            {
                ...
                else 
                {
                    ...
                    while (data_count < data_amount)
                    {
                        ...
                        buttons1[data_count % 512] = value;
                        data_count++;
                        
                        // When we have a full block, write it to the card
                        if (data_count % 512 == 0)
                        {
                            sd_card.write_data(buttons1, data_count);
                        }
                    }
                    
                    // Write the rest of the data to the card
                    sd_card.write_data(buttons1, data_count - (data_count % 512) + 512);
                }
