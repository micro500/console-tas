 ...
 * Should work even with standard 40MHz rated MCUs.
 * Code should be portable to (and work well on) 80MHz MCUs.
 *
 * The external 8MHz crystal is optional and is used for USB only.
 ...
 * ---
 *
 * $Id: main.c 43 2013-12-31 00:35:22Z true $
 */

#define VER_MAJOR		1
#define VER_MINOR		0
#define VER_SUBMINOR	'e'

#define SYS_FREQ		42000000L
#define PB_FREQ		SYS_FREQ


// enable usb? (normally set in project settings)
// #define USE_USB
...
UINT8			data_write[4];
UINT8			data_read[4];			// need separate lookup for multitap

UINT8			data_mask[2];			// bitmask for normalization; 0 = p1, 1 = p2
...
volatile UINT8		ioaux_state[2] = {0, 0};	// ioaux state

/* mode */
UINT8			mode = MODE_NES;		// 0 = nes, 1 = 5-wire snes, 2 = 7-wire snes
UINT8			startup = START_NORMAL;	// ghost latch on poweron, not reset.
...
void parse_uart()
{
	...
	switch (uart_rx_buf[0]) {
		...
		case 's': {	// nes skip first ghost latch
			if (uart_rx_len >= 2) {
				if (uart_rx_buf[1] == '0') {
					startup = START_NORMAL;
				} else {
					startup = START_SKIPFIRST;
				}
				done = 1;
			}
			break;
		...
		case 'v': {	// unit version information
			SerSendDataByte('V');
...
static inline void nesLatch(void)
{
	...
	// we do this after the clockout so we don't slow it down.
	// if we are ignoring input because of the startup latch glitch, then quit now.
	if (startup) {
		startup--;

		// we clear the bits for tidiness...probably not required
		PORTClearBits(iopin_port, iopin_data0[0]);
		PORTClearBits(iopin_port, iopin_data0[1]);
		PORTClearBits(iopin_port, iopin_data1[0]);
		PORTClearBits(iopin_port, iopin_data1[1]);
...
void nesClock(int player)
{
	...
	// have we clocked out all of our data? data0 length is always assumed >= data1
	...
	// clock out
	if ((data[0][p][data_read[p]] << clock[p]) & (1 << 31)) {
		PORTClearBits(iopin_port, iopin_data0[player]);
	} else {
		PORTSetBits(iopin_port, iopin_data0[player]);
	}

	if ((data[1][p][data_read[p]] << clock[p]) & (1 << 31)) {
		PORTClearBits(iopin_port, iopin_data1[player]);
	} else {
...
int main(int argc, char** argv) {
	...
	*/
	...
	// default controller bitmask setup
	data_mask[0] = 8;
	data_mask[1] = 8;
